    ...
    return GetMaterialApp(
      ...
      theme: ThemeData(
        ...
        appBarTheme: const AppBarTheme(
          ...
          titleTextStyle: TextStyle(fontFamily: 'Poppins', fontSize: 17, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E)),
        ),
        navigationBarTheme: NavigationBarThemeData(
          backgroundColor: Colors.white,
          indicatorColor: const Color(0xFF8095E4).withOpacity(0.15),
          labelTextStyle: WidgetStateProperty.all(const TextStyle(fontSize: 11, fontFamily: 'Poppins')),
        ),
        elevatedButtonTheme: ElevatedButtonThemeData(
          style: ElevatedButton.styleFrom(
            backgroundColor: const Color(0xFF8095E4), foregroundColor: Colors.white, elevation: 0,
...
class AppRoutes {
  static final pages = [
    ...
    GetPage(name: '/admin/tenant-detail', page: () => const TenantDetailScreen()),
    GetPage(name: '/admin/saran-kesan', page: () => const SaranKesanScreen()),
    GetPage(name: '/tenant/dashboard', page: () => const TenantDashboardScreen()),
...
class _SplashScreenState extends State<SplashScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 2200), () {
      ...
      Get.offAllNamed(auth.isLoggedIn ? (auth.isAdmin ? '/admin/dashboard' : '/tenant/dashboard') : '/login');
    });
  }
        ...
        child: FadeTransition(opacity: _fade, child: ScaleTransition(scale: _scale, child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.home_work_rounded, color: Color(0xFF8095E4), size: 56),
            ),
            const SizedBox(height: 24),
            const Text('Kostify', style: TextStyle(fontSize: 38, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -0.5)),
            const SizedBox(height: 6),
            const Text('Smart Boarding Management System', style: TextStyle(fontSize: 12, color: Colors.white70)),
